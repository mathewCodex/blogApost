...
const SideNav: React.FC<sideNavProps> = ({ currentUser }) => {
	...
	const SIDENAV_ITEMS: SideNavItem[] = [
		{
			title: "Home",
			path: `/profiles/user?${currentUser?.id}`,
			icon: <BiHome size={24} />,
		},
		{
			title: "Activities",
			path: `/profiles/dashboard?${currentUser?.id}`,
		...
		},
		{
			...
			subMenuItems: [
				...
				{ title: "Pending approval", path: "/pending-approval" },
			],
		},
		{
			title: "Notification",
		...
		},
		{
			...
			submenu: true,
			subMenuItems: [
				{ title: "Edit Account", path: `/profiles/onBoarding?${currentUser?.id}` },
